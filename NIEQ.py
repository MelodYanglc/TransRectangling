...
def _get_patches_generic(img, patch_size, is_train, stride):
    ...
    img2 = resize(img, output_shape=(int(img.shape[0] * 0.5), int(img.shape[1] * 0.5)))
...
def niqe(inputImgData):
    ...
    params = scipy.io.loadmat(r'D:\PyProjects\imageStitching\DeepRectangling_SwinTransformer_Warp\utils\niqe_image_params.mat')
...
#     avg_niqe = 0
#
#     path = r'D:\dataResource\HomographyEstimate\DIR-D\DIR-D'
#     pathGT = os.path.join(path, 'testing\\gt')
#     pathITest = os.path.join(path, 'testing\\gt')
#     index_all = list(sorted([x.split('.')[0] for x in os.listdir(pathGT)]))
#     for i in range(0, length):
#         idx = index_all[i]
#         gt_img = cv2.imread(os.path.join(pathGT, str(idx) + '.jpg'))
#         test_img = cv2.imread(os.path.join(pathITest, str(idx) + '.jpg'))
#
#         ref = gt_img[:, :, 0]  # ref
#         dis = test_img[:, :, 0]  # dis
#         niqeV = niqe(ref)
#         avg_niqe += niqeV
#
#         print("index:",(i+1),"Each NIEQ:", niqeV)
#     print("NIEQ:", avg_niqe/length)
...
if __name__ == '__main__':
    ...
    path = r'D:\dataResource\HomographyEstimate\DIR-D\DIR-D'
    pathGT = os.path.join(path, 'testing\\ours')
    index_all = list(sorted([x.split('.')[0] for x in os.listdir(pathGT)]))
    for i in range(0, length):
        idx = index_all[i]
        gt_img = cv2.imread(os.path.join(pathGT, str(idx) + '.jpg'))
