...
def transformer(U, source, target, out_size):
    ...
    def _interpolate(im, x, y, out_size):
        ...
        if torch.cuda.is_available():
            dim2 = dim2.to(device)#.cuda()
            dim1 = dim1.to(device)#.cuda()
            y0 = y0.to(device)#.cuda()
            y1 = y1.to(device)#.cuda()
            x0 = x0.to(device)#.cuda()
            x1 = x1.to(device)#.cuda()
            base = base.to(device)#.cuda()
    ...
    def _meshgrid(height, width, source):
        ...
        if torch.cuda.is_available():
            px = px.to(device)#.cuda()
            py = py.to(device)#.cuda()
